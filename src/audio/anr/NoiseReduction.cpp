...
// sndfile is not needed for the core NR algorithm (only TrackUtils uses it)
// #include <sndfile.h>
...
public:
    ...
    // ── Streaming helpers (called by NoiseReduction streaming methods) ────────
    void resetForStream()    { StartNewTrack(); }

    void feedStream(Statistics& stats, float* buf, size_t len, OutputTrack& out)
    {
        mInSampleCount += len;
        ProcessSamples(stats, buf, len, &out);
    }

    void flushStream(Statistics& stats, OutputTrack& out)
    {
        FinishTrack(stats, &out);
    }
...
bool NoiseReduction::hasProfile() const
{
    return mStatistics && mStatistics->mTotalWindows > 0;
}

void NoiseReduction::beginStream()
{
    NoiseReduction::Settings s(mSettings);
    s.mDoProfile = false;
    mStreamWorker = std::make_unique<NoiseReductionWorker>(s, mSampleRate);
    mStreamWorker->resetForStream();
}

void NoiseReduction::feedStream(const float* in, size_t n, std::vector<float>& out)
{
    out.clear();
    if (!mStreamWorker || !hasProfile() || n == 0)
        return;

    // Copy to a mutable buffer (ProcessSamples takes float*)
    std::vector<float> buf(in, in + n);

    OutputTrack localOut;
    mStreamWorker->feedStream(*mStatistics, buf.data(), n, localOut);

    const size_t produced = localOut.Length();
    out.resize(produced);
    if (produced > 0)
        std::copy(localOut.Buffer().begin(),
                  localOut.Buffer().begin() + produced,
                  out.begin());
}

void NoiseReduction::endStream(std::vector<float>& out)
{
    out.clear();
    if (!mStreamWorker)
        return;
    ...
    OutputTrack localOut;
    mStreamWorker->flushStream(*mStatistics, localOut);
